...
if($pub = mysqli_fetch_array($publicacion2)){
    ...
    $textwpp = $pub['mensajedewpp'];
    $titulo = $pub['titulo'];
    $vibilidad = $pub['vibilidad'];
    $tienda = $pub['tienda'];
...
<select class="form-select text-center" id="tienda" aria-label="Default select example">
  <option  value="<?php echo $tienda; ?>"><?php echo $tienda; ?></option>
...
<input type="text" id="textowpp" value="<?php echo $textwpp; ?>" placeholder="Texto de mensaje whatssap">
<input type="text" id="title" value="<?php echo $titulo; ?>" placeholder="Titulo de la publicidad">
...
<select class="selectvisibility" id="visibilidad" aria-label="Default select example">
  <option  value="<?php echo $titulo; ?>"><?php if($vibilidad == 1){ echo "Visible";}else{ echo "No visible";} ?></option>
...
$(document).ready(function () {
    $.ajax({
        type: 'POST',
        url: '../../funciones/modificalinkpublicidad.php',
        data: {
            'id': '<?php echo $_GET['id'] ; ?>'
